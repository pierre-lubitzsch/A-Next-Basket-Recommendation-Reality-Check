...
def get_data_loader_temporal_split(history_path, future_path, data_type, batch_size, item_embedding_matrix):
    """
    Args:
        data_path: str
        data_type: str, 'train'/'validate'/'test'
    ...
    dataset = TemporalSplitSetDataset(history_path, future_path, item_embedding_matrix=item_embedding_matrix, key=data_type)
    print(f'{data_type} data length -> {len(dataset)}')
    data_loader = DataLoader(dataset=dataset,
                             batch_size=batch_size,
                             shuffle=False,
...
class TemporalSplitSetDataset(SetDataset):
    def __init__(self, history_path, future_path, item_embedding_matrix, key=None):
        """
        Args:
            data_path: str
            key: str
